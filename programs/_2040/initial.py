...
class Initial(share.TestSequence):
    ...
    def _step_sec_check(self, dev, mes):
        ...
        self.measure(('dmm_VoutExt', 'dmm_SDOff', 'dmm_GreenOn', ), timeout=5)
    ...
    def _step_dcpower_on(self, dev, mes):
        ...
        self.measure(
            ('dmm_DCmin', 'dmm_VccDC', 'dmm_Vout', 'dmm_GreenOn',
             'dmm_RedDCOff'), timeout=5)
        dev['dcl_Vout'].output(1.0, output=True, delay=1.0)
        mes['dmm_Vout'](timeout=5)
        dev['dcs_dcin'].output(40.0)
        self.measure(('dmm_DCmax', 'dmm_VccDC', 'dmm_Vout', ), timeout=5)
        dev['dcs_dcin'].output(25.0)
        self.measure(
            ('dmm_DCtyp', 'dmm_VccDC', 'dmm_Vout', 'ramp_OCP'), timeout=5)
        dev['dcl_Vout'].output(4.1, delay=0.5)
        self.measure(('dmm_SDOn', 'dmm_RedDCOn', ), timeout=5)
    ...
    def _step_acpower_on(self, dev, mes):
        ...
        self.measure(
            ('dmm_ACmin', 'dmm_VbusMin', 'dmm_VccAC', 'dmm_Vout',
             'dmm_GreenOn', 'dmm_RedACOff'), timeout=15)
        dev['dcl_Vout'].output(2.0, delay=1.0)
        mes['dmm_Vout'](timeout=5)
        dev['acsource'].output(voltage=265.0, delay=0.5)
        self.measure(('dmm_ACmax', 'dmm_VccAC', 'dmm_Vout', ), timeout=5)
...
class Devices(share.Devices):
    ...
    def open(self):
        ...
        self['dcs_dcin'] = tester.DCSourceParallel(
            (self['dcs_dcin1'], self['dcs_dcin2'],
            self['dcs_dcin3'], self['dcs_dcin4'], ))
