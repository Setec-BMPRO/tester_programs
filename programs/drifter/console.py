...
class Console(share.console.Base):
    ...
    cmd_data = {
        "UNLOCK": parameter.Boolean(
            "XDEADBEA7 UNLOCK", writeable=True, readable=False, write_format="{1}"
        ),
        "NVDEFAULT": parameter.Boolean(
            "NV-WRITE-DEFAULT", writeable=True, readable=False, write_format="{1}"
        ),
        ...
        "APS_DISABLE": parameter.Float(
            "APS-DISABLE", writeable=True, readable=False, write_format="{0} {1}"
        ),
        ...
        "V_FACTOR": parameter.Float("X-CAL-FACTOR-VOLTS", read_format="{0} X?"),
        "I_FACTOR": parameter.Float("X-CAL-FACTOR-CURRENT", read_format="{0} X?"),
    }
