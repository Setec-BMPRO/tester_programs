...
def teststep(func):
    ...
    @wraps(func)
    def new_func(self):
        return func(self, self.logdev, self.logdev.j35, self.meas)
    return new_func
...
class Initial(tester.TestSequence):     # pylint:disable=R0902
    ...
    @teststep
    def _step_prepare(self, dev, j35, mes):
    ...
    def _step_initialise_arm(self, dev, j35, mes):
        ...
        """
        j35.open()
        j35.brand(limit.ARM_HW_VER, self.sernum, dev.rla_reset)
        j35.manual_mode(True)     # Start the change to manual mode
        mes.arm_swver.measure()

    @teststep
    def _step_aux(self, dev, j35, mes):
    ...
    @teststep
    def _step_solar(self, dev, j35, mes):
        """Test Solar input."""
    ...
    @teststep
    def _step_output(self, dev, j35, mes):
