...
class Main(share.TestSequence):
    ...
    def _step_checker(self, dev, mes):
        ...
        self.measure(
            ('dmm_12V', 'dmm_5Va', 'dmm_5Vb', 'dmm_5Vc', 'dmm_5Vd',
             'dmm_5Ve', ),
            timeout=5)
    ...
    def _step_dso(self, dev, mes):
        ...
        """
        self.measure(
            ('dmm_shld1off', 'dmm_shld2off', 'dmm_shld3off',
             'dmm_shld4off', ),
             timeout=5)
        for meas in mes['dso_subchan']:
            meas(timeout=5.0)
            self.measure(
                ('dmm_shld1on', 'dmm_shld2on', 'dmm_shld3on',
                 'dmm_shld4on', ),
                 timeout=5)
    ...
    def _step_discharge(self, dev, mes):
        ...
        dis.opc()
        self.measure(
            ('dmm_Disch1On', 'dmm_Disch2On', 'dmm_Disch3On', ),
            timeout=5)
        dis.set_off()
        dis.opc()
        self.measure(
            ('dmm_Disch1Off', 'dmm_Disch2Off', 'dmm_Disch3Off', ),
            timeout=5)
